...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+mysqlconnector://root:password@localhost:3306/byte_card'
...
@app.route('/cartoes/<int:id>/cancelar')
def cancela_cartao(id):
    use_cases.cancela_cartao(id)
    return redirect('/cartoes/lista')


@app.route('/cartoes/<int:id>/ativar')
def ativa_cartao(id):
    use_cases.ativa_cartao(id)
    return redirect('/cartoes/lista')


@app.route('/cartoes/<int:id>/limite')
